// EditClassModal component for editing existing dance classes
...
interface EditClassModalProps {
  isOpen: boolean;
  onClose: () => void;
  onClassUpdated: (updatedClass: DanceClass) => void;
...
export const EditClassModal: React.FC<EditClassModalProps> = ({
  ...
  // Populate form when modal opens with class data
  useEffect(() => {
    if (isOpen && danceClass) {
      setFormData({
        title: danceClass.title,
        style: danceClass.style,
        dateTime: danceClass.dateTime.slice(0, 16), // Format for datetime-local input
        location: danceClass.location,
        description: danceClass.description,
        price: danceClass.price?.toString() || '',
        videoLink: danceClass.videoLink || '',
        rsvpLink: danceClass.rsvpLink || '',
        flyer: danceClass.flyer || ''
      });
      setErrors({});
    }
  }, [isOpen, danceClass]);
